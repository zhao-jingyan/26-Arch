...
- **rs1_addr / rs2_addr 归属**：作为 forward 判定 key，`rs*_addr` 生命周期「出生在 Decoder、死在 EX」，归入 `id_2_fwd` 而**不**进 `INST_CTX` 或 `ID_2_EX`。
...
本轮采用**最小 stall 实现**：pipeline_stall 只吃两个"忙"信号（IF 取指没就绪、MEM 访存没就绪），然后向所有 stage 均匀施加；不做 load-use stall。RAW forward 路径由独立的 `Forward_Unit` 承担（见 §3.7），与 Control_Unit 解耦。
...
- **与 Forward_Unit 分工**：RAW 数据冒险由 §3.7 的 `Forward_Unit` 单独处理；Control_Unit 仅关心 stall 与跳转。
- **load-use 暂不 stall**：若 EX/MEM 是 load 且下一拍 EX 从 `ex_2_fwd` forward，其 `ex_result` 是地址而非数据；当前接受该不正确行为，待补 load-use stall。
...
- **Load-use 例外**：当 EX/MEM 是 load 指令时，`ex_2_fwd.ex_result` 是访存地址而非 load 数据，distance-1 forward 会给出错误值。本轮不做 load-use stall，接受该错误；distance-2 及更远的 load 则已在 `mem_2_wb.rd_data` 对齐，forward 正确。
...
- **hazard / forwarding**：RAW 已由 §3.7 `Forward_Unit`（distance-1/2）+ §3.2.2 RegFile write-during-read bypass（distance-3）覆盖。仍待办：
  - **load-use stall**：EX/MEM 是 load 且 rd 匹配当前 EX 指令 rs1/rs2 时，应注入一拍 bubble；当前未实现，`ex_2_fwd.ex_result` 对 load 会给出错误（地址而非数据）。落地时可在 Control_Unit 内基于 `ex_2_fwd.rd_addr / id_2_fwd.rs*_addr` 与"EX 当前是 load"的判定拉起 `stall_if / stall_id`，并在 EX/MEM 寄存器插 bubble。
  - **`ID_2_CTRL` 字段扩充**：为 load-use 检测准备 `rs1_addr / rs2_addr / is_rs1_used / is_rs2_used / is_store_src_used` 等 flag，取代当前 `placeholder`。
