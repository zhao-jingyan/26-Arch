...
| system_input | input | `clk` + `rst_n` |
| `stall` | input | 流水线暂停 |
| `flush` | input | 跳转命中时清空 IF/ID 寄存器（`= flush_if_id`） |
...
- **IF/ID 寄存器更新条件**：优先级 `rst_n > flush > (is_inst_ready && !stall)`；`flush` 高时写零（NOP），否则 ready & 非 stall 时 latch，再否则保持
- **跳转时 IF/ID**：`flush_if_id` 由 Control_Unit 在 `ex_pc_should_jump` 命中拍拉高，把已 speculative 取入 IF/ID 的 wrong-path 指令清掉，避免它走到 ID/EX 进而进入 EX 产生副作用
...
| `insert_bubble` | input | 为高时向 ID/EX 流水线寄存器写入 0（NOP bubble）；用于 load-use stall **以及**跳转命中时清掉 ID 内的 wrong-path 指令 |
...
- **ID/EX 寄存器更新条件**：优先级 `rst_n > insert_bubble > !stall`；复位同步清零，`insert_bubble` 时写入 0（NOP bubble），`!stall` 时正常 latch 新数据。`inst_ctx` 与 `id_2_ex` / `id_2_fwd` 物理上为同一组寄存器，只是按语义拆成多个端口。
- **flush 语义**：跳转命中时 Control_Unit 把 `insert_bubble` 拉高，ID/EX 写零；与 IF/ID 的 `flush_if_id` 配合，覆盖 EX 段 branch/jal/jalr 决出后两个流水槽的 wrong-path 指令。`insert_bubble` 因此是 **load-use 与 jump-flush 共用**的同一根线（含义统一为"下一拍 ID/EX 给 NOP"）。
...
独立控制层模块，聚合 IF / ID / EX / MEM 对控制层的反馈信号，产生每个 stage 的 `stall`、ID/EX 的 `insert_bubble` 与 IF/ID 的 `flush_if_id`，并把 EX 的跳转反馈转发给 IF。**纯组合**，无 system_input。
...
- **jump flush**：EX 段决出 `ex_pc_should_jump` 当拍，IF 已 speculative 取了 PC+4 进 IF/ID，ID 也已 decode 了上一拍的 PC+4 → ID/EX 出口。Control_Unit 同拍把 `flush_if_id` 与 `insert_bubble` 都拉高，把 IF/ID 与 ID/EX 两槽 wrong-path 指令清成 NOP；PC 由 `pc_should_jump` 透传过去重定向到 `ex_pc_jump_address`。
...
| `insert_bubble` | output | 供 `ID_Stage.insert_bubble`；= `(load_use_hazard \|\| ex_pc_should_jump) && !pipeline_stall`（load-use 与 jump-flush 共用） |
| `flush_if_id` | output | 供 `IF_Stage.flush`；= `ex_pc_should_jump && !pipeline_stall`（仅跳转拉高，load-use 不拉） |
...
- **跳转 flush 时序**：cycle T 时 EX 决出 `ex_pc_should_jump=1`：(1) PC 当拍取 `ex_pc_jump_address`（PC 内部 `pc_should_jump > stall` 优先级保证）；(2) `flush_if_id=1` → 下沿 IF/ID 写 0；(3) `insert_bubble=1` → 下沿 ID/EX 写 0。cycle T+1 时新 PC 已生效，IF/ID 与 ID/EX 都是 NOP，wrong-path 不会进入 EX。两槽 flush 对应 EX 段决出跳转的固定 2-cycle 气泡惩罚（branch/jal/jalr 同样代价）。
- **load-use 与 jump 互斥性**：EX 位指令同时是 load 又能产生跳转是不可能的（load 不写 PC），所以 `load_use_hazard && ex_pc_should_jump = 0` 恒成立；`insert_bubble` 同时含两个 OR 项不会引发语义冲突。
- **跳转透传**：`pc_should_jump / pc_jump_address` 为 EX 组合直出，Control_Unit 当拍转给 IF，不加寄存器。
- **与 Forward_Unit 分工**：Control_Unit 只判 stall / bubble / flush，Forward_Unit 只解数据通路；检测逻辑（比较 rd vs rs）虽然形式相近但各自独立实现，便于日后分别扩展（branch hazard、MEM→MEM bypass 等）。
...
flowchart LR
    ...
    Ctrl -->|stall_if, flush_if_id| IFStage
