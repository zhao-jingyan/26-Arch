...
-include $(DESIGN_DIR)/verilate/Makefile.opt.mk
...
EMU_CXXFLAGS += -g -std=c++2a -Wall -I$(EMU_CSRC_DIR) -I$(SIM_CSRC_DIR) -I$(DIFFTEST_CSRC_DIR)
EMU_CXXFLAGS += $(EMU_OPT_FLAGS)
EMU_LDFLAGS  += $(EMU_LDFLAGS_OPT)
EMU_CXXFLAGS += -DVERILATOR -Wno-maybe-uninitialized  -DNUM_CORES=$(NUM_CORES)
EMU_LDFLAGS  += -lpthread -lz
...
ifneq ($(SDL2_CONFIG),)
# sdl2-config --cflags 给的是 .../include/SDL2，但源码用 #include <SDL2/SDL.h>，需要 .../include
SDL2_PREFIX  := $(shell $(SDL2_CONFIG) --prefix)
EMU_CXXFLAGS += -I$(SDL2_PREFIX)/include -D_THREAD_SAFE
EMU_LDFLAGS  += $(shell $(SDL2_CONFIG) --libs)
else
EMU_CXXFLAGS += -DDIFFTEST_NO_SDL
endif
...
build_emu_local: $(EMU_MK)
	@echo "\n[$(EMU_CXX)] Compiling C++ files..." >> $(TIMELOG)
	@date -R | tee -a $(TIMELOG)
	$(TIME_CMD) $(MAKE) VM_PARALLEL_BUILDS=1 \
		CXX="$(EMU_CXX)" OPT_FAST="$(EMU_OPT_FLAGS)" \
		-C $(<D) -f $(<F) $(EMU_COMPILE_FILTER)
